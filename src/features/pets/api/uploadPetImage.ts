...
/**
 * Compress an image client-side and upload it to Supabase Storage under `folder`.
...
export async function uploadImage(file: File, folder = 'pets'): Promise<string> {
  ...
  const path = `${folder}/${year}/${id}.webp`;
...
export function uploadPetImage(file: File): Promise<string> {
