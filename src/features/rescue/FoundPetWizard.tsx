...
type Step = 'intro' | 'shelters' | 'help' | 'immediate' | 'form' | 'done';

/** Tiny "Paso X de Y" progress hint. */
function StepHint({ n }: { n: number }) {
  return <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-faint">Paso {n} de 4</p>;
}
...
export function FoundPetWizard() {
  ...
  async function handlePublished(pet: Pet) {
    ...
    // automatically (requester_type=finder). Never block the success screen on it.
    if (wantsImmediate && help.length > 0) {
      const labels = help
      ...
      try {
        await createNeed.mutateAsync({
          city: pet.location || zone || 'Sin especificar',
          reference: null,
      ...
      <div className="animate-fade">
        ...
        {needCreated && (
          <p className="mt-4 rounded-xl border border-[#D4E7D8] bg-[#EAF3EC] px-3.5 py-3 text-center text-[12.5px] text-forest-dark">
            Publicamos tu solicitud de apoyo en <Link to="/necesidades" className="font-bold underline">Necesidades</Link>.
          </p>
        )}

        <MatchSection
          title="Refugios y hogares que pueden ayudar"
          subtitle={published.location ? `Cerca de ${published.location}` : undefined}
          seeAllTo="/refugios"
        >
          <ShelterMatchList city={published.location} />
        </MatchSection>
    ...
    return (
      <div className="animate-fade">
        <PageHeading
          ...
          subtitle="Una foto clara y la zona ayudan a que el rescate avance más rápido."
        />
        <StepHint n={4} />
        <PetForm
          status="found"
  ...
  if (step === 'help') {
    return (
      <div className="animate-fade">
        ...
        <Button fullWidth className="mt-6" onClick={() => setStep('immediate')}>
          Continuar
        </Button>
        <BackLink onClick={() => setStep('intro')} />
      </div>
      ...
      <div className="animate-fade">
        ...
        {wantsImmediate && (
          <div className="mt-4 space-y-3 rounded-2xl border border-sand-200 bg-[#FBF8F1] p-3.5">
            <p className="text-[12.5px] text-muted">Solo dos datos para reducir fricción:</p>
            <Input
